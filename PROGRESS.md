...
### Session 12 (3D Voxel Simulation)

- **`src/simulation3d.rs`** — new module with `Grid3D { width, height, depth, cells }`, 3D index formula (`y*width*depth + z*width + x`), `step_simulation_3d` (gravity-biased 6-direction delta-buffer diffusion; down=0.5, lateral=0.1×4, up=0.05), `build_depth_pressure_3d` (scans each X,Z column bottom-to-top accumulating upward push), `step_objects_3d` (3-pass MoveIntent with 3D pressure forces).
- **Axis convention** — Y=0 is ground floor, Y=height-1 is top. Gravity pulls in −Y (toward lower Y). Pressure accumulates upward (bottom-to-top), so objects float toward higher Y.
- **`GridDirty` message** — fired by `simulate_flow`, `simulate_objects`, and any grid mutation; consumed by `render_grid_3d` to rebuild the 8 face-culled meshes.
- **`ActiveLayer` resource** — tracks which Y layer (horizontal XZ slice) the player is editing.
- **Face-culled voxel rendering** — `render.rs` replaced: 8 `(Mesh3d, MeshMaterial3d)` entities (wall, rock, sand, spring, drain, building, water, object). `build_voxel_mesh` emits only faces adjacent to transparent cells. Rebuilds on `GridDirty`.
- **`find_cursor_cell_3d`** — ray-casts cursor to the active Y-layer plane; returns `(x, y, z)`.
- **Hover gizmo** — 4-line square outline at the active layer height, drawn with `gizmos.line`.
- **Perspective camera** — `OrthographicProjection` replaced with `PerspectiveProjection { fov: PI/4 }`. Scroll changes orbit distance (not scale). Pan uses `distance * 0.001` as sensitivity.
- **Layer UI** — Layer `-/+` buttons in panel; `update_layer_label` shows current layer.
- **3D level format** — `LevelData` gains `depth: Option<usize>`, `CellPlacement` gains `z: Option<usize>`. Old 2D levels load with depth=1.
- **`levels/valley-flood.json`** — 40×20×40 valley with Rock floor, Rock walls on all 4 XZ edges, Spring at (1,1,1). Generated by a `#[test]` helper in `levels.rs`.
- **`persistence.rs`** — `SaveData` gains `depth` field; validates width×height×depth on load.
- **25/25 unit tests pass** — 7 new 3D tests (Grid3D struct, simulation conservation, depth pressure, object buoyancy).
...
src/
  main.rs         — app setup, registers all plugins
  grid.rs         — GridPlugin, Grid3D resource, ActiveLayer, GridDirty, simulation systems, handle_input
  simulation.rs   — Cell/Grid types (2D, kept for 2D unit tests), step_simulation, step_objects
  simulation3d.rs — Grid3D, step_simulation_3d, build_depth_pressure_3d, step_objects_3d
  textures.rs     — TexturesPlugin, TextureAssets resource, programmatic froth textures
  persistence.rs  — save/load Grid3D to JSON files
  levels.rs       — LevelsPlugin, CurrentLevel resource, load_level, LevelData/CellPlacement (3D)
  render.rs       — RenderPlugin, VoxelMeshes, build_voxel_mesh, find_cursor_cell_3d, draw_hover_cursor
  camera.rs       — CameraPlugin, perspective orbit camera
  ui.rs           — UiPlugin, all tool/layer/flow buttons
  undo.rs         — UndoStack (3D: records x,y,z)
  config.rs       — AppConfig (inc. grid_depth)
levels/
  valley-flood.json  — Valley Flood (40×20×40, Rock bowl, Spring at corner)
  coastal-bowl.json  — Coastal Bowl (legacy 2D level, still loads)
```

## Where We Left Off

3D voxel simulation complete on branch `feat/3d-simulation`. Ready to test with `cargo run`.
...
- **Pressure heatmap view** — re-add 3D heatmap rendering (ViewMode::Pressure was removed)
- **More levels** — design levels that showcase 3D flooding (canyons, tiered terrain)
- **Performance** — profile at 40×20×40; consider only rebuilding changed mesh categories
- **Sound effects** — water flow, splash, object placement
